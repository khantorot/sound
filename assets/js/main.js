...
function showSound() {
  ...
  function hideSound() {
    ...
    setTimeout(function () {
      ...
      document.querySelector('.work_page').classList.remove('work_page_show');
    }, 500);
  }
  ...
  for (key in projects) {
    if (name == projects[key].name) {
      ...
      info += '<li><img src="./content/images/play.png" alt="' + projects[key].name + '"></li>';
      info += '</ul>';

      info += '<ul class="info_box">';
      info += '<h3>' + projects[key].name + '</h3>';
